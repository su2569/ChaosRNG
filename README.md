...
- **消息内容**: OneBot 接收的消息原文仅用于即时哈希计算，**缓冲区只存储 SHA-256 哈希值**，不保留原文
- **环境变量**: 只读取键名，值使用 **完整 64 字符 SHA-256 + 32 字节随机盐** 哈希后存储，不截断
- **所有网络内容**: 微博/知乎/B站/GitHub 等平台内容经哈希后存入缓冲区，不存储原文
- **内存安全**: 敏感数据（消息、环境变量值）使用 `bytearray` 作为中间缓冲区，处理后立即用零覆盖，最小化在内存中的驻留时间

### 权限与风险

- **内存模式**: `self` 模式仅读取本进程内存，安全；`all` 模式需要 root 且可能被安全软件标记
- **抓包**: 使用 scapy 或原始 socket，需要管理员/root 权限
- **输入监听**: pynput 全局监听可能需要额外权限配置
- **内存读取攻击**: 本程序以高权限（管理员/root）运行时，理论上低权限攻击者可能通过内存读取获取程序内存中的敏感数据。虽然已采取 bytearray 清零等措施，但 **Python 的内存管理机制无法完全防止字符串等不可变对象的内存残留**。对于极高安全要求场景，建议：
  1. 使用 `self` 内存模式（不读取其他进程内存）
  2. 在隔离环境/沙箱中运行
  3. 考虑使用内存加密或禁用 OneBot/内容平台等涉及敏感数据的熵源
